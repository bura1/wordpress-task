...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'my-project')}>

			<HelpModal
				type='column'
				buttonLabel={__('How to use a Column?', 'my-project')}
				modalLabel={__('Column', 'my-project')}
			/>
			...
			<Responsive label={<IconLabel icon={icons.order} label={__('Order', 'my-project')} />}>
				{breakpoints.map((keyName, index) => {
					...
					return (
						<Fragment key={keyName}>
							...
							/>
							{index === 0 &&
								<p className='es-decorative-text'>{__('Needs to be set on all columns!', 'my-project')}</p>
							}
						</Fragment>
					);
				})}
			...
			<Responsive label={<IconLabel icon={icons.verticalAlign} label={__('Vertical align', 'my-project')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
			...
			<Responsive label={<IconLabel icon={icons.horizontalAlign} label={__('Horizontal align', 'my-project')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
