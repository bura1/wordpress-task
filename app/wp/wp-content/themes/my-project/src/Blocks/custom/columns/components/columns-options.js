...
export const ColumnsOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Columns', 'my-project')}>
			<HelpModal
				type="columns"
				buttonLabel={__('How to use Columns?', 'my-project')}
				modalLabel={__('Columns', 'my-project')}
			/>

			<br /><br />

			<Responsive label={<IconLabel icon={icons.gutter} label={__('Column spacing', 'my-project')} />}>
			...
			<Responsive label={<IconLabel icon={icons.verticalSpacing} label={__('Row spacing', 'my-project')} />}>
				{breakpoints.map((keyName) => {
					const point = ucfirst(keyName);
