		...
		<>
			...
			{buttonUse &&
				<>
					{showButtonColor &&
						<ColorPaletteCustom
							label={<IconLabel icon={icons.color} label={__('Color', 'my-project')} />}
							value={buttonColor}
					...
					{showButtonIsLink &&
						<IconToggle
							icon={icons.link}
							label={__('Display as link', 'my-project')}
					...
					{showButtonSize && !buttonIsLink &&
						<SimpleVerticalSingleSelect
							label={<IconLabel icon={icons.size} label={__('Size', 'my-project')} />}
							options={sizeOptions}
						/>
					}
					...
					{showButtonWidth && !buttonIsLink &&
						<SimpleVerticalSingleSelect
							label={<IconLabel icon={icons.width} label={__('Width', 'my-project')} />}
					...
					{showButtonIsAnchor &&
						<IconToggle
							icon={icons.anchor}
							label={__('Anchor', 'my-project')}
							checked={buttonIsAnchor}
							onChange={(value) => setAttributes({ [getAttrKey('buttonIsAnchor', attributes, manifest)]: value })}
							help={__('Make sure to set the URL as an ID of the element you want to target, e.g. #my-block.', 'my-project')}
						/>
					}

					{showButtonId &&
						<TextControl
							label={<IconLabel icon={icons.id} label={__('Unique identifier', 'my-project')} />}
							value={buttonId}
